...
export default {
  ...
  computed: {
    ...
    table() {
      ...
      let table = [];
      const data =
        this.dataBefore.x_axis.length > this.dataAfter.x_axis.length
          ? this.dataBefore
          : this.dataAfter;
      const tariffs = [...this.tariffs, "trans__EnergyUnits"];
      table = data.x_axis.map((date, i) => {
    ...
    dateBefore() {
      ...
      return this.dataBefore.x_axis[0].split("-")[0];
    },
    dateAfter() {
      if (!this.dataAfter.x_axis.length) return null;
      return this.dataAfter.x_axis[0].split("-")[0];
    }
  },
  watch: {
    device() {
    ...
    async getData() {
      ...
      try {
        let [dataBefore, dataAfter] = await Promise.all([
          ...
          axios.get(
            `Chart/ConsumptionByYear?DeviceID=${this.device.id}&DateFrom=${this.widgetParams.date.date[1]}&EnergyGuideCode=${this.widgetParams.energyType}&testimony=${this.widgetParams.isTestimony}`
          )
        ]);
        ...
        if (
          ...
          this.fail = this.$chkNoDatas(dataBefore, dataAfter, "year");
